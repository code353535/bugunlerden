...
<section class="grid grid-cols-1 md:grid-cols-4 gap-8 min-h-16 mt-8 md:px-0 px-4">
  <div v-for="post in filteredPosts" :key="post.id" id="remainingPosts" class="col-span-3 grid grid-cols-1 gap-8">
      ...
      <div class="relative w-full md:w-1/3 h-[250px] md:h-auto overflow-hidden group">
            <a v-if="post.imageUrl" :href="`/post/${post.slug}`">
              <img
                ...
                :alt="post.altText"
                loading="lazy"
                class="w-full object-cover object-center md:object-top max-h-[250px] min-h-[250px]transition-transform duration-500 group-hover:scale-105"
              />
            </a>
          </div>
